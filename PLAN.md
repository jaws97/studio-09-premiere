...
**Slice 6 (Sep 21) — done:** live on Vercel (https://studio-09-premiere-6onh.vercel.app) with the Supabase store (`funfriday-db`, all objects prefixed `s09_`, bucket `s09-photos`); setup is scripted in `scripts/setup-supabase.mjs` · 100-guest rehearsal against the live site: 0 failures, 0 duplicate seats, bravos exact, p50 294ms · all 27 no-face posters generated (2 credits each; film 03 retitled "Prathimoana") · `/host/posters` review gallery + `docs/poster-review.jpg` · Vercel Analytics · repo made public at the organiser's request.

**Waiting on feedback (as of Sep 21):** (1) poster redo list from the organiser's reviewers — my own shortlist: #13 murky, #22 neon sign text, #09 robot too close to its source; (2) announcer voice pick from `docs/voice-samples/` (Arthur / Desmond / Holden, ElevenLabs engine, 0.6 credits per line) plus pronunciation notes for titles and Indian names. Then: one more test line → record all 35 → drop into `public/media/vo/`. Generator note: prompts that sit too close to a specific film get rejected as "nsfw"; make the scene more original and they pass.
